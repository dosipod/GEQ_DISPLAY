...
inline void renderSpectrumEffects(uint8_t targetMode, int i, int xPos, int barW, int baseW, int baseH, int barH, int rawVal, uint8_t fallSpeed, uint8_t globalAverageVolume, uint8_t localBins[], uint8_t previousBins[]) {
  ...
  // Determine rendering color according to selected mode
  if (targetMode == 0)      color = gfx->color565(0, 229, 255); // Default Cyan
  else if (targetMode == 1) color = RGB565_CYAN;
  ...
  else if (targetMode == 11) color = gfx->color565(200, 50, i * 16);
  else if (targetMode == 13) color = (rawVal > 150) ? RGB565_MAGENTA : gfx->color565(0, rawVal, i * 8);
  else if (targetMode == 14) color = (globalAverageVolume > 140) ? RGB565_BLUE : RGB565_GREEN;
  else if (targetMode == 15) color = gfx->color565(rawVal, 255 - rawVal, 255);
  ...
  if (targetMode == 1) { 
    ...
    gfx->writeFillRect(x, y, w, h, color); 
    gfx->writeFillRect(x, h, w, baseH - h, RGB565_BLACK);
  } 
    ...
    // Standard bottom-up vertical visualizer bars (Modes 0, 2, 4, 6, 8, 9, 10, 13)
